	...
	<div class="background">
		...
		<div class="tBody">
			...
			<input style="display: none;" id="helpDivId"
				value="<c:out value="${helpId}" />" />
			<div class="leftNav" align="left">
				<c:forEach items="${listHelp}" var="thelp">
					<ul>
						<li><a id="title<c:out value="${thelp.id}" />"
							href="helps/getAllHelps?helpId=<c:out value="${thelp.id}" />"
							target="_top"><c:out value="${thelp.title}" /> </a></li>
					</ul>
				</c:forEach>
			</div>
			<div class="rightDiv">
						...
						<div class="contentDiv">
							...
							<div align="center"
								...
								<c:if test="${sessionScope.userInfo.roleId==6}">
									<a
										href="help_ManageGoUpdate.action?thelp.id=<c:out value="${thelp.id}" />">更新</a>
								</c:if>
								<c:if test="${sessionScope.userInfo.roleId!=6}"></c:if>
							</div>
							<div align="left">
								<c:out value="${thelp.content}" default="expression" escapeXml="false" />
							</div>
							<div align="right" style="margin-bottom: 50px;font-size: 13px;">
								发布于：
								<fmt:formatDate value="${thelp.newtime}" type="date"/>
							</div>
						</div>
						<div class="pageNav" align="left">
							<c:if test="${thelp.id==1}"></c:if>
							<c:if test="${thelp.id!=1}">
								<a
									href="helps/getAllHelps?helpId=<c:out value="${thelp.id-1}" />">&lt;&lt;上一篇</a>
							</c:if>
						</div>
						<div class="pageNav" align="right">
							<c:if test="${thelp.id==helpSize}"></c:if>
							<c:if test="${thelp.id!=helpSize}">
								<a
									href="helps/getAllHelps?helpId=<c:out value="${thelp.id+1}" />">下一篇&gt;&gt;</a>
							</c:if>
						</div>
					</div>
