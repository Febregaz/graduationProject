...
<title><c:out value="${sessionScope.user.nickname}" />的帖子</title>
		...
		<div class="tbody">
			<div class="smallNav" align="left">
				...
				<div style="float: left;line-height:24px;">
					&nbsp;&gt;&nbsp;<a href="NC-JSP/home/index.jsp">论坛</a>&nbsp;&gt;&nbsp;
			...
			<div>
				<div class="editMenu">
					<input type="button" value="发帖" class="butt" style="width: 80px;"
						...
						onclick="window.location.href = '/topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=1'"
				...
				<div class="pageNav" align="right">
					...
					<c:if test="${pageBean.totalPages == 2}">
						<c:if test="${pageBean.currentPage==1}">
							...
							<button disabled="disabled">1</button>
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=2"><button>2</button>
							</a>
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=<c:out value="${pageBean.currentPage+1}"/>"><button
						...
						<c:if test="${pageBean.currentPage==2}">
							...
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=1"><button>1</button>
							</a>
					...
					<c:if test="${pageBean.totalPages == 3}">
						<c:if test="${pageBean.currentPage==1}">
							...
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=3"><button>3</button>
							</a>
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=<c:out value="${pageBean.currentPage+1}"/>"><button
									style="width: 80px;">下一页</button> </a>
						...
						<c:if test="${pageBean.currentPage==2}">
							<a
									...
									style="width: 80px;">上一页</button> </a>
							<a
									href="topics/getAllNiceTopics?getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=1"><button>1</button>
							</a>
							<button disabled="disabled">2</button>
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=3"><button>3</button>
					...
					<c:if test="${pageBean.totalPages > 3}">
						<c:if test="${pagaBean.currentPage==1}">
							...
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=<c:out value="${pageBean.totalPages}" />"><button>
						...
						<c:if test="${pageBean.currentPage==pageBean.totalPages}">
							...
							<a><button disabled="disabled">...</button> </a>
							<a
									href="topics/getTopicsByUserId?userId=<c:out value="${sessionScope.user.id}" />&&nowPage=<c:out value="${pageBean.totalPages-1}" />"><button>
				...
				<script type="text/javascript">
					function goPage() {
						...
						if (selectedPage != 0 && selectedPage != currentPage) {
							document.goPageForm.action = "/topics/getTopicsByUserId?userId=<c:out value='${sessionScope.user.id}' />&&nowPage="
									+ selectedPage;
							goPageForm.submit();
						}
				...
				<div class="pageGo" align="right">
					<form
						action="user_GoUserAllTopics.action?user.id=<c:out value="${sessionScope.user.id}" />"
				...
				<table align="left" class="listTopicStyle">
					<c:forEach items="${listTopic}" var="topic">
						<tr>
							...
							<td align="center" width="60px;"><c:if
									test="${topic.niceTopic==1}">是</c:if> <c:if test="${topic.niceTopic==0}">
										否</c:if></td>
							<td align="center" width="40px" style="font-size: 13px;"><c:out
									value="${topic.integral}" /></td>
							<td align="center" width="40px" style="font-size: 13px;"><c:out
									value="${topic.countComment}" /></td>
							<td style="font-size: 12px;" width="110px;" align="right"><fmt:formatDate value="${topic.topicTime}" type="date"/></td>
						</tr>
					</c:forEach>
				</table>
			</div>
