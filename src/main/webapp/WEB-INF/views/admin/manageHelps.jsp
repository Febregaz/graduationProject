...
<script type="text/javascript">
	function goPage() {
		...
		if (selectedPage != 0 && selectedPage != currentPage) {
			document.goPageForm.action = "helps/manageAll?nowPage="
					+ selectedPage;
			goPageForm.submit();
		}
		...
		<div class="right">
			...
			<div style="float: left;width:100px;margin-left: 10px;">
				<input type="submit" value="发表" class="butt" style="width: 80px;"
					onclick="window.location.href='NC-JSP/admin/addHelp.jsp'" />
			</div>
			<div class="pageNav" align="right">
				...
				<c:if test="${pageBean.totalPages == 2}">
					<c:if test="${pageBean.currentPage==1}">
						...
						<button disabled="disabled">1</button>
						<a
								href="helps/manageAll?nowPage=2"><button>2</button>
						</a>
						<a
								href="helps/manageAll?nowPage=<c:out value="${pageBean.currentPage+1}" />"><button
					...
					<c:if test="${pageBean.currentPage==2}">
						...
						<a
								href="helps/manageAll?nowPage=1"><button>1</button>
						</a>
				...
				<c:if test="${pageBean.totalPages == 3}">
					<c:if test="${pageBean.currentPage==1}">
						...
						<a
								href="helps/manageAll?nowPage=3"><button>3</button>
						</a>
						<a
								href="helps/manageAll?nowPage=<c:out value="${pageBean.currentPage+1}" />"><button
								style="width: 80px;">下一页</button> </a>
				...
				<c:if test="${pageBean.totalPages > 3}">
					<c:if test="${pageBean.currentPage==1}">
						...
						<a
								href="helps/manageAll?nowPage=<c:out value="${pageBean.totalPages}" />"><button>
					...
					<c:if test="${pageBean.currentPage==pageBean.totalPages}">
						...
						<a><button disabled="disabled">...</button> </a>
						<a
								href="helps/manageAll?nowPage=<c:out value="${page.currentPage-1}" />"><button>
				...
				<c:forEach items="${listHelp}" var="help">
					<tr style="height:26px">
						<td style="border-left: 1px solid silver;"><div
								...
								<c:out value="${help.title}" />
							</div>
						</td>
						<td><fmt:formatDate value="${help.newtime}" type="date"/>
						</td>
						<td style="font-size: 12px;color: blue;"><a
							href="helps/getAllHelps?helpId=<c:out value="${help.id}" />"
							target="_top">查看详细&gt;&gt;</a>
						</td>
